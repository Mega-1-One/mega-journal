...
import { Plus, Flame, ShieldCheck, BookMarked, ArrowRight } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  // Win/Loss Pie Data
  const pieData = [
    { name: 'Wins', value: analytics.winningTrades, color: '#00D2FF' },
    { name: 'Losses', value: analytics.losingTrades, color: '#EF4444' },
    { name: 'Breakeven', value: analytics.breakEvenTrades, color: '#4A6B8A' },
  ];
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="btn-cyan-accent text-xs px-4 py-2 rounded-xl shadow-cyan-glow flex items-center gap-1.5 font-heading font-black"
          >
      ...
      <div className="custom-card p-6 border-l-4 border-brand-cyan bg-bg-card flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          <span className="text-xs font-bold text-brand-cyan uppercase tracking-widest font-heading">NET PERFORMANCE</span>
          <div className="flex items-baseline gap-3">
            <span className={`text-4xl font-black font-mono-num font-heading ${analytics.netPnL >= 0 ? 'text-brand-cyan' : 'text-loss'}`}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 font-mono-num border-t md:border-t-0 md:border-l border-bg-border pt-4 md:pt-0 md:pl-6">
          ...
          <div>
            ...
            <span className="text-lg font-bold text-brand-cyan">{analytics.winRate}%</span>
          </div>
          <div>
      ...
      {/* STRATEGY & ADHERENCE INSIGHTS BAR */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="custom-card p-4 flex items-center justify-between">
          ...
          <Link href="/strategies" className="p-2 rounded-xl bg-brand-cyan/10 text-brand-cyan hover:bg-brand-cyan/20 transition-colors">
        ...
        <div className="custom-card p-4 flex items-center justify-between">
          <div className="space-y-0.5">
            ...
            <span className="text-sm font-bold text-brand-cyan font-heading">{topPlaybook.name}</span>
          </div>
          <Link href="/playbooks" className="p-2 rounded-xl bg-brand-cyan/10 text-brand-cyan hover:bg-brand-cyan/20 transition-colors">
        ...
        <div className="custom-card p-4 flex items-center justify-between font-mono-num">
          <div className="space-y-0.5">
            ...
            <span className="text-sm font-bold text-brand-cyan font-heading">{adherenceComparison.followed.winRate}%</span>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 font-mono-num">
        ...
        <div className="custom-card p-4">
          ...
          <span className="text-xl font-black text-brand-cyan font-heading">${analytics.averageWin}</span>
        </div>
        <div className="custom-card p-4">
        ...
        <div className="custom-card p-4">
          ...
          <span className="text-xl font-black text-brand-cyan font-heading">${analytics.expectancy}</span>
        </div>
        <div className="custom-card p-4">
          <span className="text-[10px] font-bold text-text-muted uppercase block font-heading mb-1">Win Streak</span>
          <span className="text-xl font-black text-brand-cyan font-heading flex items-center gap-1">
            <Flame className="w-4 h-4 fill-brand-cyan" /> {analytics.currentWinStreak}
          </span>
        </div>
      </div>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={equityCurveData}>
                ...
                <XAxis dataKey="date" stroke="#4A6B8A" fontSize={10} tickLine={false} />
                <YAxis stroke="#4A6B8A" fontSize={10} tickLine={false} domain={['auto', 'auto']} />
                <Tooltip
                  contentStyle={{ backgroundColor: '#00132C', borderColor: '#002B5B', borderRadius: '8px', color: '#FFFFFF', fontSize: '11px' }}
                />
                <Area type="monotone" dataKey="balance" stroke="#00D2FF" strokeWidth={2.5} fillOpacity={1} fill="url(#equityCyanGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 custom-card p-5 flex flex-col justify-between space-y-4">
          ...
          <div className="h-44 w-full flex items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip contentStyle={{ backgroundColor: '#00132C', borderColor: '#002B5B', color: '#FFFFFF', fontSize: '11px' }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs font-mono-num">
            <div className="p-2 rounded-xl bg-bg-nested border border-bg-border">
              ...
              <span className="font-bold text-brand-cyan">{analytics.winningTrades}</span>
            </div>
            <div className="p-2 rounded-xl bg-bg-nested border border-bg-border">
